...
$bg_img        = get_theme_file_uri('assets/images/hero-rochas-fallback.jpg'); // fallback
$bg_video      = get_theme_file_uri('assets/videos/contato-hero.mp4'); // opcional
...
<main id="primary" class="site-main">
  ...
  <section class="relative min-h-[78vh] md:min-h-[84vh] flex items-center justify-center overflow-hidden bg-secondary">
    ...
    <img src="<?php echo esc_url($bg_img); ?>" alt="" class="absolute inset-0 w-full h-full object-cover opacity-20" />

    <div class="absolute inset-0 bg-gradient-to-b from-secondary/35 via-secondary/70 to-secondary"></div>
    ...
    <div class="relative w-full max-w-6xl px-6 py-16 grid grid-cols-1 md:grid-cols-12 gap-8">
      ...
      <div class="md:col-span-6 text-gray-100">
        <p class="uppercase tracking-[0.22em] text-xs md:text-sm text-custom1/75 mb-4">Inteligência • Intermediação • Inspeção Técnica</p>
        <h1 class="text-4xl md:text-5xl font-bold mb-4 tracking-wide uppercase">Contato</h1>
        <p class="text-base md:text-lg text-custom1/90 mb-8 max-w-xl leading-relaxed">
        ...
        <ul class="space-y-3 text-sm md:text-base">
          <li>
            <strong class="text-custom1/90">E-mail:</strong>
            <a class="underline hover:opacity-80" href="mailto:valeria@tradeexpansion.com.br">valeria@tradeexpansion.com.br</a>
            <span class="text-custom1/60"> / </span>
            <a class="underline hover:opacity-80" href="mailto:vitor@tradeexpansion.com.br">vitor@tradeexpansion.com.br</a>
          </li>
          <li>
            <strong class="text-custom1/90">WhatsApp:</strong>
            <a class="underline hover:opacity-80" href="<?php echo esc_url($te_whatsapp); ?>" target="_blank" rel="noopener">Falar com um especialista</a>
            <span class="text-custom1/70"> (resposta rápida)</span>
          </li>
          <li class="text-custom1/75">
            <strong class="text-custom1/90">Expectativa:</strong> resposta em até <span class="text-custom1">1 dia útil</span> com próximos passos.
          </li>
        </ul>
      </div>
      ...
      <div class="md:col-span-6">
        <div class="bg-custom1/95 backdrop-blur rounded-2xl shadow-2xl p-6 md:p-10 border border-accent/15">
          <?php if ( $te_success ) : ?>
          ...
          <form method="post" action="<?php echo esc_url( get_permalink() ); ?>" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="block text-sm font-medium text-secondary mb-1">Nome*</label>
                <input class="w-full rounded-lg border border-secondary/20 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/60 focus:border-accent/40"
            ...
            <div>
              <label class="block text-sm font-medium text-secondary mb-1">Empresa</label>
              <input class="w-full rounded-lg border border-secondary/20 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/60 focus:border-accent/40"
                     type="text" name="te_company" value="<?php echo isset($company) ? esc_attr($company) : ''; ?>">
            </div>

            <div>
              <label class="block text-sm font-medium text-secondary mb-1">Assunto*</label>
              <input class="w-full rounded-lg border border-secondary/20 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-accent/60 focus:border-accent/40"
                     type="text" name="te_subject" required value="<?php echo isset($subject) ? esc_attr($subject) : ''; ?>">
            </div>

            <div>
              <label class="block text-sm font-medium text-secondary mb-1">Mensagem*</label>
              <textarea class="w-full rounded-lg border border-secondary/20 px-3 py-2 h-32 focus:outline-none focus:ring-2 focus:ring-accent/60 focus:border-accent/40"
                        name="te_message" required><?php echo isset($message) ? esc_textarea($message) : ''; ?></textarea>
            </div>

            <label class="inline-flex items-start gap-2 text-sm text-secondary/90">
            ...
            <div class="flex flex-wrap gap-3 pt-2">
              <button type="submit" class="bg-secondary text-custom1 px-7 py-3 rounded-xl font-semibold border border-accent/20 hover:border-accent/40 hover:-translate-y-[1px] transition">
                Enviar mensagem
              </button>
              <a href="<?php echo esc_url($te_whatsapp); ?>" target="_blank" rel="noopener"
                 class="bg-transparent text-secondary px-7 py-3 rounded-xl font-semibold border border-secondary/25 hover:border-accent/45 hover:-translate-y-[1px] transition">
  ...
  <!-- Mapa (ajuste $map_embed_src no topo caso a localização esteja incorreta) -->
  <section class="bg-secondary py-14">
    <div class="w-full max-w-6xl mx-auto px-6">
      <div class="aspect-[16/9] w-full rounded-2xl overflow-hidden shadow-2xl border border-custom1/10">
        <iframe
