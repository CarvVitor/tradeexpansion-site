...
// Adicionar regras de reescrita para URLs multilíngues da Home (/en/, /es/ e /pt/)
add_action('init', function() {
    $front_id = get_option('page_on_front');
    if ($front_id) {
        add_rewrite_rule('^en/?$', 'index.php?page_id=' . $front_id . '&te_lang=en', 'top');
        add_rewrite_rule('^es/?$', 'index.php?page_id=' . $front_id . '&te_lang=es', 'top');
        add_rewrite_rule('^pt/?$', 'index.php?page_id=' . $front_id . '&te_lang=pt', 'top');
    } else {
        add_rewrite_rule('^en/?$', 'index.php?te_lang=en', 'top');
        add_rewrite_rule('^es/?$', 'index.php?te_lang=es', 'top');
        add_rewrite_rule('^pt/?$', 'index.php?te_lang=pt', 'top');
    }
});
...
// Impedir que o WordPress faça redirecionamento 301 para a URL original da home e apague o idioma
add_filter('redirect_canonical', function($redirect_url, $requested_url) {
    if (get_query_var('te_lang')) {
        return false;
    }
    return $redirect_url;
}, 10, 2);

// Forçar o <title> correto no WordPress com suporte a idiomas
add_filter('pre_get_document_title', function($title) {
    if (is_front_page() || is_home()) {
        $lang = get_query_var('te_lang') ?: 'pt';
        
        if ($lang === 'en') {
            return 'Brazilian Natural Stone Export & Sourcing | Trade Expansion';
        } elseif ($lang === 'es') {
            return 'Piedras Naturales para Exportación | Trade Expansion';
        } else {
            return 'Rochas Naturais para Exportação | Trade Expansion';
        }
    }
    return $title;
}, 999);
